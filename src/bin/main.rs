use zkvm_from_scratch::{VmState, encode_hex, StarkProver, StarkVerifier, Prover, Verifier};
...
fn demo_basic_vm() {
    ...
    vm.memory.load_program(&program, 0).expect("Failed to load program");
...
fn demo_zero_knowledge_proof() {
    ...
    let trace = prover.generate_execution_trace(&mut vm, 3)
    ...
    println!("  - {} intermediate values captured", 
             trace.steps.iter().map(|s| s.intermediate_values.len()).sum::<usize>());
    ...
    println!("  - Trace commitment: {}", encode_hex(&proof.trace_commitment));
    ...
    println!("  - Verification ~{}x faster than execution", 
